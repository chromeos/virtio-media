// Copyright 2022-2024 The ChromiumOS Authors
...
use super::*;
use crate::ffmpeg::ffi::AVPictureType;
...
impl DecoderContextBuilder {
    ...
    /// Set the initial format for the context.
    ///
    /// This ensures the context has a valid resolution and pixel format right after its creation.
    pub fn set_initial_format(&self, coded_format: (u32, u32), pix_fmt: AvPixelFormat) {
        // SAFETY:
        // Safe because self.context.0 is a pointer to a live AVCodecContext allocation.
        let context = unsafe { &mut *(self.context.0) };
        context.width = coded_format.0 as i32;
        context.height = coded_format.1 as i32;
        context.coded_width = coded_format.0 as i32;
        context.coded_height = coded_format.1 as i32;
        context.pix_fmt = pix_fmt.0;
    }
...
pub const AV_PIXEL_FORMAT_YUV420P: AvPixelFormat =
    AvPixelFormat(ffi::AVPixelFormat_AV_PIX_FMT_YUV420P);
pub const AV_PIXEL_FORMAT_NV12: AvPixelFormat = AvPixelFormat(ffi::AVPixelFormat_AV_PIX_FMT_NV12);

impl From<AvPixelFormat> for ffi::AVPixelFormat {
    fn from(val: AvPixelFormat) -> Self {
        val.0
    }
}
...
impl AvPixelFormat {
    ...
    /// bytes.
    pub fn line_size(&self, width: u32, plane: usize) -> usize {
        // This should not fail if the format is correct.
        av_image_line_size(*self, width, plane).unwrap_or_default()
    }
    ...
    pub fn plane_sizes<I: IntoIterator<Item = u32>>(
        ...
        av_image_plane_sizes(*self, linesizes, height).unwrap_or_default()
    }
}
...
impl AvCodecContext {
    ...
    ///
    /// Returns `Received` if `frame` has been filled with the next decoded frame, `TryAgain` if
    ...
    ///
    /// Returns `Received` if `packet` has been filled with encoded data, `TryAgain` if
...
impl AvBufferSource for Vec<u8> {
    fn as_ptr(&self) -> *const u8 {
        self.as_ptr()
    }

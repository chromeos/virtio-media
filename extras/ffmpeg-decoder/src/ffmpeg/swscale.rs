...
use crate::ffmpeg::avcodec::AvError;
use crate::ffmpeg::avcodec::AvFrame;
use crate::ffmpeg::avcodec::Dimensions;
...
#[derive(Debug, ThisError)]
pub enum SwConverterCreationError {
...
impl SwConverter {
    ...
    pub fn new(
        ...
        src_pix_format: ffi::AVPixelFormat,
        dst_pix_format: ffi::AVPixelFormat,
    ) -> Result<Self, SwConverterCreationError> {
        // SAFETY:
        ...
        if sws_context.is_null() {
            Err(SwConverterCreationError::CreationFailure)
        } else {
            Ok(Self {
                sws_context,
                src_pix_format,
